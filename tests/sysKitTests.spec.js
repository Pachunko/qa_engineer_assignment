...
(0, test_1.test)('cannot delete user due to privileges', async ({ page }) => {
    ...
    await (0, test_1.expect)(actionFailedNotification).toContainText('Action completed with errors. ');
    // clicks on notification
    await actionFailedNotification.click();
    // validate error message (insufficient privileges)
    await (0, test_1.expect)(page.locator('.status-text')).toContainText('Insufficient privileges to complete the operation.');
